...
new #[Title('User Details')] class extends Component {
    ...
    // Helper functions that can be used in the view
    ...
    // Pass data to the view using with() method
...
<div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
                    ...
                    <div class="flex-shrink-0">
                        <div class="avatar">
                            <div class="w-24 h-24 rounded-full">
                                <img src="{{ $user->profile_photo_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&color=7F9CF5&background=EBF4FF' }}" alt="{{ $user->name }}" />
                            </div>
                        </div>
                    </div>
            ...
            <!-- Profile Information -->
            @if($userProfile)
                <x-card title="{{ $userProfile['type'] }} Profile">
                    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                        @if($userProfile['type'] === 'Student' && $user->programEnrollments->count() > 0)
                            <div class="md:col-span-2">
                        ...
                        @if($userProfile['type'] === 'Parent' && $user->children->count() > 0)
                            <div class="md:col-span-2">
                                ...
                                <div class="space-y-3">
                                    @foreach($user->children as $child)
                                        <div class="p-3 border rounded-md bg-gray-50">
                                            <div class="flex items-center justify-between">
                                                <div>
                                                    <div class="font-medium">{{ $child->full_name ?? $child->name }}</div>
        ...
        <div class="space-y-6">
            ...
            <x-card title="Related">
                <div class="space-y-2">
                    @if(method_exists($user, 'isStudent') && $user->isStudent() && $user->programEnrollments->count() > 0)
                    ...
                    @if(method_exists($user, 'isParent') && $user->isParent() && $user->children->count() > 0)
