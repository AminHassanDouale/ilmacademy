...
new #[Title('Users Management')] class extends Component {
    ...
    protected function loadStats(): void
    {
        try {
            ...
            // Get role counts with error handling for Spatie Permission
            try {
                ...
                // Fallback if Spatie Permission is not set up
                $adminCount = 0;
                $teacherCount = 0;
                $parentCount = 0;
                $studentCount = 0;
        ...
        return User::query()
            ...
            ->when($this->roleFilter, function (Builder $query) {
                try {
                    ...
                    // If Spatie Permission is not available, skip role filtering
                }
            })
                ...
                <tbody>
                    @forelse($users as $user)
                        <tr class="hover">
                            ...
                            <td>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $this->getStatusColor($user->status ?? 'active') }}">
