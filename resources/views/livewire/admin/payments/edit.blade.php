<?php
// resources/views/livewire/admin/payments/edit.blade.php
...
new #[Title('Edit Payment')] class extends Component {
    ...
    {
        $this->payment = $payment->load(['student.user', 'student.curriculum']);

        // Populate form with existing data
        $this->student_id = (string) $payment->student_id;
        ...
        $this->due_date = $payment->due_date ? $payment->due_date->format('Y-m-d') : '';
    ...
    public function save(): void
    {
        ...
        try {
            ...
            $originalData = $this->payment->toArray();
            $changes = [];
            ...
            if (!empty($changes)) {
                activity()
                    ...
                    ->withProperties([
                        'changes' => $changes,
                        'student_name' => $this->payment->student->user->name ?? 'Unknown'
                    ])
                    ->log('Payment updated');
            }
...
<div>
    <!-- Page header -->
    <x-header title="Edit Payment" separator>
        <x-slot:middle>
